...
class AbstractCanalClient {
  ...
  protected var clientModel = "simple"
  protected var ip = "127.0.0.1"
  protected var port = 11111
  protected var zkAddress = ""
  ...
  def this(destination: String, connector: CanalConnector) {
    ...
    if (connector == null)
      this.connector = if ("simple" == clientModel) CanalConnectors.newSingleConnector(new InetSocketAddress(ip, port), destination, username, password)
    else CanalConnectors.newClusterConnector(zkAddress, destination, username, password)
  ...
  protected def buildPositionForDump(entry: CanalEntry.Entry): String = {
    ...
    val format: SimpleDateFormat = new SimpleDateFormat(AbstractCanalClient.DATE_FORMAT)
    entry.getHeader.getLogfileName + ":" + entry.getHeader.getLogfileOffset + ":"+ entry.getHeader.getExecuteTime + "(" + format.format(date) + ")"
  }
  ...
  protected def printEntry(entrys: List[CanalEntry.Entry]) {
    ...
    for (entry <- entrys) {
      ...
      val delayTime: Long = System.currentTimeMillis - executeTime
      if (
